...
def worker(pdf_file, data_dir, output_dir):
    ...
    for page_id in tqdm(range(len(pdf.pages))):
        ...
        for figure in  this_page.objects.get('image', []):
            figure_bbox = (float(figure['x0']), float(figure['top']), float(figure['x1']), float(figure['bottom']))
            figure_bbox = tuple([min(1000, max(0, int(t / int(this_page.width) * 1000))) for t in figure_bbox])
            x0, y0, x1, y1 = [int(t * int(this_page.width) / 1000) for t in figure_bbox]
            for x in range(x0, x1):
                for y in range(y0, y1):
                    anno_img[x, y] = [0, 0, 0]
            tokens.append(('##LTFigure##',) + figure_bbox + ('default',))
        for line in  this_page.objects.get('line', []):
            line_bbox = (float(line['x0']), float(line['top']), float(line['x1']), float(line['bottom']))
            line_bbox = tuple([min(1000, max(0, int(t / int(this_page.width) * 1000))) for t in line_bbox])
            x0, y0, x1, y1 = [int(t * int(this_page.width) / 1000) for t in line_bbox]
            for x in range(x0, x1 + 1):
                for y in range(y0, y1 + 1):
                    anno_img[x, y] = [0, 0, 0]
            tokens.append(('##LTLine##',) + line_bbox + ('default',))
        ...
        with open(os.path.join(output_dir, pdf_file.replace('.pdf', '') + '_{}.txt'.format(str(page_id))),
                    ...
                    encoding='utf8') as fp:
            for token in tokens:
                fp.write('\t'.join([str(t) for t in token]) + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--data_dir",
        default="/home/sebastiank/Videos",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--output_dir",
        default="/home/sebastiank/Videos/output",
        type=str,
        required=False,
